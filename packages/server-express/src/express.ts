...
type Options = { onLog?: (err: any, req: Request) => void; traceId?: (req: Request) => string | undefined; };

export function errorHandler(opts?: Options){
  return (err: any, req: Request, res: Response, _next: NextFunction) => {
    const traceId = opts?.traceId?.(req);
    opts?.onLog?.(err, req);
    if (err instanceof AppError) return res.status(err.status).json(fail(err.code, err.message, err.details, traceId));
